...
export default function MetricsGrid({ total, refs }) {
  return (
    <div className="grid grid-cols-2 gap-5 w-fit mx-auto" style={{ gridTemplateColumns: 'repeat(2, 200px)' }}>
      ...
      {/* Total references */}
      <div className="flex flex-col items-center gap-0.5 text-center rounded-md border border-success/25 bg-success/4 backdrop-blur-panel px-5 py-2 shadow-[0_4px_20px_rgba(16,185,129,0.25)]">
        <span className="text-xs font-medium text-text-secondary">총 사용 참조 횟수</span>
        <span className="text-[22px] font-bold font-mono text-white" style={{ textShadow: '0 0 10px rgba(16,185,129,0.25)' }}>
          {refs}
        </span>
      </div>
    </div>
