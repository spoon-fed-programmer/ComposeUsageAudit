...
### 3.5 리포트 뷰어 (Report Viewer) 기능 및 UI 사양
* **데이터 비동기 로딩 및 다국어 지원:**
  * 백엔드 서버 없이 브라우저 단에서 JSON 리포트 데이터를 비동기로 로드하여 동적으로 렌더링하며, 한국어(KO) 및 영어(EN)의 즉각적인 언어 전환을 지원합니다.
* **사이드바(Sidebar) 주기 필터 및 레이아웃:**
  * 사이드바 최상단에 **주기 필터 버튼 탭(일별/주간/월간/연간)**이 배치되어 클릭 시 해당 주기의 리포트 인덱스 파일로 스위칭합니다.
  * 필터 탭 바로 아래에 `"리포트 이력"` 헤더와 `"전체 이력 보기"` 토글 버튼이 정렬되어 배치됩니다.
* **세부 사용처 표시 정밀화:**
  * 컴포넌트 세부 정보 패널에서 호출 클래스명 바로 오른쪽에 해당 사용처의 모듈명 및 소스세트가 괄호 속에 공백 없이 표기됩니다. (예: `(app/main)`, `(vas/dpaper/main)`)
* **전체 이력 보기 (AllHistoryMatrix) 다중 보기 모드:**
  * 리포트 전체 주기의 참조 횟수 추이를 표시하는 매트릭스 테이블에서 두 가지 모드로 전환하여 시각화할 수 있는 전환 버튼 탭이 탑재되어 있습니다.
    1. **컴포넌트별 모드 (기본)**: 세로 행에 공통 소스 파일 및 개별 컴포넌트명을 배치하여 개별 컴포넌트 단위의 참조 수 증감 트렌드를 표시합니다.
    2. **모듈별 모드**: 세로 행을 모듈명(`module_name`)으로 피벗하여, 시간 흐름에 따라 각 모듈별로 공통 컴포넌트를 사용한 총 참조 횟수의 변화와 증감 트렌드를 분석합니다. (과거 분석 시점에 모듈 정보가 수집되지 않은 옛 기록들은 기본 모듈인 `"app"` 모듈로 보정 적용됩니다).
